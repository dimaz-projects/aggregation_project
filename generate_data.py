...
def main(rows):
    ...
    log.info('Start generation data')

    from concurrent.futures import ThreadPoolExecutor

    with ThreadPoolExecutor(2) as pool:
        pool.submit(generate_csv, 'example_data_log.csv', rows)
        pool.submit(generate_xml, 'example_data_log.xml', rows)

    log.info('Done generation data')
...
if __name__=='__main__':
    ...
    with ThreadPoolExecutor(2) as pool:
        pool.submit(generate_csv, 'example_data_log.csv', 10000)
        pool.submit(generate_xml, 'example_data_log.xml', 10000)
